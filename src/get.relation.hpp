...
namespace rlim {
                                                                        ...
                                                                        const int numUnits)
  {
    ...
    retCode = XmimGetRecordsDouble(handle, 1, const_cast<char**>(&relname),  numColumns, xmimColumns, fromDate, toDate, fromTime, toTime, numUnits, units, XMIM_FILL_NAN, XMIM_FILL_NAN, XMIM_SKIP_ALL_NAN, 0, XMIM_LIMIT_BY_RECORDS, &numRecords, &dates, &values);
    ...
    return xmim2fts<TDATE,TDATA,TSDIM,TSDATABACKEND,DatePolicy>(numRecords, numColumns, values, dates, colnames);
  }
} // namespace rlim
